...
/*Table patient_status_ARV contains all patients and their status*/
	...
	DELIMITER $$
	DROP PROCEDURE IF EXISTS insertion_obs_by_day$$
	CREATE PROCEDURE insertion_obs_by_day()
		BEGIN
		 /*Started DML queries*/
	...
	DELIMITER $$
	DROP PROCEDURE IF EXISTS insertion_patient_by_day$$
	CREATE PROCEDURE insertion_patient_by_day()
		BEGIN
		 /*Started DML queries*/
			/* insert data to patient table */
			SET SQL_SAFE_UPDATES = 0;
	...
	DELIMITER $$
	DROP PROCEDURE IF EXISTS patient_dispensing_by_day$$
	CREATE PROCEDURE patient_dispensing_by_day()
		BEGIN
		 /*Started DML queries*/
			/* insert data to patient table */
			SET SQL_SAFE_UPDATES = 0;
		...
		/* Insert for dispensing drugs */
				...
				INSERT into patient_dispensing_day
					...
					ob.encounter_id,ob.location_id,ob.value_coded,DATE(ob2.obs_datetime), now(), ob.voided
				...
				/*update rx_or_prophy for table patient_dispensing*/
					update isanteplus.patient_dispensing_day pdisp, isanteplus.obs_by_day ob1, 
	...
	DELIMITER $$
	DROP PROCEDURE IF EXISTS patient_laboratory_dml_day$$
	CREATE PROCEDURE patient_laboratory_dml_day()
		BEGIN
		 /*Started DML queries*/
			SET SQL_SAFE_UPDATES = 0;
...
DELIMITER ;
	...
	DELIMITER $$
	DROP PROCEDURE IF EXISTS virological_tests_day$$
	CREATE PROCEDURE virological_tests_day()
	BEGIN
...
/*END of virological_tests table*/
	...
	DELIMITER $$
	DROP PROCEDURE IF EXISTS patient_status_arv_day$$
	CREATE PROCEDURE patient_status_arv_day()
	BEGIN
     /* DECLARE myIndex INT;
	...
	*/
	...
	/* Condition D - Des ARV prescrits en prophylaxie
		...
		*/
		...
		from patient_dispensing_day pdisp, (select ppres.patient_id, 
					MAX(ppres.visit_date) as visit_date FROM patient_dispensing_day ppres 
	...
	DROP table IF EXISTS patient_pcr_temp_day;
	CREATE TEMPORARY TABLE IF NOT EXISTS patient_pcr_temp_day
	...
	patient_id IN (SELECT ppt.patient_id FROM patient_pcr_temp_day ppt WHERE 
	ppt.concept_id IN (844,1030) AND ppt.value_coded IN(703,1301));
	
	truncate table patient_pcr_temp_day;
...
INSERT INTO patient_status_arv_day(patient_id,id_status,start_date,encounter_id,last_updated_date, date_started_status)
	...
	isanteplus.obs_by_day ob, (SELECT pvi.patient_id, MAX(DATE(pvi.date_started)) as visit_date 
...
INSERT INTO patient_status_arv_day(patient_id,id_status,start_date,encounter_id,last_updated_date,date_started_status)
	...
	INSERT INTO patient_status_arv_day(patient_id,id_status,start_date,encounter_id,
	last_updated_date, date_started_status)
	SELECT v.patient_id,3 as id_status, MAX(DATE(v.date_started)) AS start_date, 
	enc.encounter_id as encounter_id, now(), now()
	...
	WHERE enc.encounter_type = entype.encounter_type_id
	AND enc.patient_id = ob.patient_id
	AND enc.encounter_id = ob.encounter_id
	AND enc.patient_id = parv.patient_id
	...
	DELETE FROM patient_status_arv_day WHERE 
...
DELIMITER ;
	...
	DELIMITER $$
	DROP PROCEDURE IF EXISTS isanteplusregimen_dml_day$$
	CREATE PROCEDURE isanteplusregimen_dml_day()
		BEGIN
			SET SQL_SAFE_UPDATES = 0;
			/*SET FOREIGN_KEY_CHECKS = 0;*/
	...
	DELIMITER $$
	DROP PROCEDURE IF EXISTS call_all_procedure_day$$
	CREATE PROCEDURE call_all_procedure_day()
	BEGIN
	
		call insertion_obs_by_day();
		call insertion_patient_by_day();
		call patient_dispensing_by_day();
		call patient_laboratory_dml_day();
		call virological_tests_day();
		call patient_status_arv_day();
		call isanteplusregimen_dml_day();
	...
	CREATE EVENT if not exists patient_status_arv_day_event
	ON SCHEDULE EVERY 10 MINUTE
	 STARTS now()
		DO
		call call_all_procedure_day();
